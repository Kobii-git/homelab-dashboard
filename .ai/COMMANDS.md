...
| Docker first-run installer | `./scripts/install-docker.sh` (`--configure-only`, `--lan-http`, `--https-proxy`) | `scripts/install-docker.sh` | DECLARED | Writes private `.env`; default mode builds and starts a local image. Fresh examples select private HTTP. |
