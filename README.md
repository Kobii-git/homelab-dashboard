...
- **Dashboard** - operations header, Lab Vitals host metrics with host detail drill-downs, expanded Daily Briefing, favorites strip, drag-and-drop card layout, grid/list density toggle, status filters, and an attention strip for anything that is down
...
- **Services** - manual catalog for apps, websites, Docker services, VMs, servers, and other devices, with common homelab templates and duplicate actions
- **Health checks** - HTTP, TCP, ping, and SSL checks with latest status, latency, failure reason, thresholds, and check history
- **Admin** - password change, Glances host monitors, runtime health diagnostics, public status page, build info, and demo-data controls
