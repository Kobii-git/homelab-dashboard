...
export function ServiceLauncher({ data, onManage }: { data: AppData; onManage: () => void }) {
  ...
  useEffect(() => {
    ...
    document.getElementById(`service-${pendingServiceId}`)?.focus();
    setPendingServiceId(null);
  }, [pendingServiceId, query]);
  ...
  return <main className="view-shell service-launcher">
    ...
    <HealthStrip items={[serviceHealth(resources, resource => { setQuery(""); setPendingServiceId(resource.id); })]} />
    ...
    {groups.map(group => {
      ...
      return <section className="launcher-group" key={group.id}>{group.id !== "ungrouped" && <h3>{group.name}</h3>}<div className="launcher-grid">
        {items.map(r => <article key={r.id} className="launcher-service" id={`service-${r.id}`} tabIndex={-1}>
          {r.url ? <a className="launcher-open" href={r.url} target="_blank" rel="noopener noreferrer"><ServiceIcon resource={r} size={40} /><span><strong>{r.name}</strong><small>{r.description || r.kind}</small></span><ArrowUpRight size={15} aria-hidden="true" /></a> : <div className="launcher-open"><ServiceIcon resource={r} size={40} /><span><strong>{r.name}</strong><small>{r.description || r.kind}</small></span></div>}
